...
PE::STATUS PE::GetExportTable(void* pFileBuffer, ExportInfo& out_ExpInfo) {
	...
	if (IsValid(pFileBuffer, pNtHeader) == PE_STATUS_SUCCESS) {
		...
		// 3. 检查是否存在导出表
		if (DataDir.VirtualAddress == 0 || DataDir.Size == 0) return PE_STATUS_GET_EXPORT_FAILURE;

		// 5. 将导出目录的 RVA 转换为文件偏移 (FOA)
		DWORD FileOffset = 0;
		FileOffset = RvaToFoa(pFileBuffer, DataDir.VirtualAddress);
		if (FileOffset == DWORD(-1)) return PE_STATUS_LOCAL_MEMORY_ALLOCATION_FAILURE;
		...
		for (FuncIndex = 0; FuncIndex < pExportDir->NumberOfFunctions; FuncIndex++) {
			...
			if (RealFunctions > 0) {
				...
				if (tmp_Pointer == nullptr) return PE_STATUS_LOCAL_MEMORY_ALLOCATION_FAILURE;
...
PE::STATUS PE::GetTlsTable(void* pFileBuffer, TLS_Info& out_tls_Info){
	...
	IMAGE_DATA_DIRECTORY DataDir = pNtHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_TLS];
	if (DataDir.VirtualAddress == 0 || DataDir.Size == 0) return PE_STATUS_SUCCESS; // 没有 TLS 表，视为成功但返回空信息

	// 定位 TLS 目录
	DWORD TLSOffset = RvaToFoa(pFileBuffer, DataDir.VirtualAddress);
...
PE::STATUS PE::MemoryToFileDump(void* pMemoryImage, const wchar_t* DumpFile)
{
	...
	if (IsValid(pMemoryImage, pNtHeader) == PE_STATUS_SUCCESS) {
		...
		#if defined(_WIN64)
			if (pNtHeader->OptionalHeader.Magic != IMAGE_NT_OPTIONAL_HDR64_MAGIC) return PE_STATUS_ARCH_MISMATCH;
			pNtHeader->OptionalHeader.ImageBase = 0x0000000140000000;
		#else
			if (pNtHeader->OptionalHeader.Magic != IMAGE_NT_OPTIONAL_HDR32_MAGIC) return PE_STATUS_ARCH_MISMATCH;
		...
		// 4.修复TLS
		IMAGE_DATA_DIRECTORY pTLSDir = pNtHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_TLS];
		if (pTLSDir.VirtualAddress == 0 || pTLSDir.Size == 0) {
			IMAGE_TLS_DIRECTORY* pTLS = reinterpret_cast<IMAGE_TLS_DIRECTORY*>(
				static_cast<char*>(pMemoryImage) + pTLSDir.VirtualAddress
			);
			DWORD TLSRVA = 0;
