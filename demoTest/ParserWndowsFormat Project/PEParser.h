...
*
* Part 1  内存管理与释放规范
* 本库部分方法在内部使用了动态内存分配，调用者在使用完毕后必须手动释放，否则将导致严重的内存泄漏：
* - 堆内存分配：调用 GetSectionName、GetExportTable、GetResourceTable、GetDebugTable 后，
*   请务必调用 free(pointer) 释放内存。
* - 虚拟内存分配：调用 Read、BuildMemoryImage 后，
*   请务必调用 VirtualFree(pointer, 0, MEM_RELEASE) 释放内存。
*
* Part 2  架构安全校验机制
* 为防止 32 位与 64 位架构不匹配导致的内存越界或崩溃，以下核心函数在执行底层逻辑前，
* 均内置了强制性的平台位数（Magic）校验：
* MemoryToFileDump、GetExportTable、FixImportTable、AddSection、
* Relocation、GetResourceTable、ClearImportFunctionName
* 
* Part 3  PE 修改与文件落盘工作流
* 本工具采用内存修改机制。当调用以下函数对内存中的 PE 缓冲区完成修改后：
* SetSizeOfImage、ClearImportFunctionName、AddSection
* 请统一调用 FileDump 函数，将最终的修改结果落盘保存为文件。
*/
...
namespace PE {
	...
	struct ExportInfo {
		...
		DWORD ExportFuncSize;		// 导出函数表总大小（字节），用于后续整体拷贝该内存块
		// 关于字段 ExportFuncSize 本人后续编写其他函数也有点没搞懂为啥要单独记这个。
	...
	STATUS FileDump(void* pFileBuffer, DWORD FileSize, const wchar_t* DumpFile);
	STATUS MemoryDump(const wchar_t* ExecuteFile, const wchar_t* DumpFile);
	STATUS MemoryToFileDump(HMODULE pMemoryImage, const wchar_t* DumpFile);
	DWORD RvaToFoa(void* pBuffer, DWORD RVA);
	DWORD FoaToRva(void* pBuffer, DWORD FOA);
	STATUS BuildMemoryImage(void* pFileBuffer, HMODULE& out_pMemoryImage);
	STATUS GetExportTable(void* pFileBuffer, ExportInfo& out_ExpInfo);
	STATUS GetExportTable(HMODULE pMemoryImage, ExportInfo& Out_ExpInfo);/*/*//*/*//*/*///*/**/
	STATUS FixImportTable(DWORD pid, HMODULE pMemoryImage/*, HMODULE pRemoteImageBase*/);
	STATUS ClearImportFunctionName(void* pFileBuffer);
	STATUS ClearImportFunctionName(HMODULE pMemoryImage);
	STATUS Relocation(HMODULE pMemoryImage, HMODULE pRemoteImage);
	STATUS SetSectionProperty(void* pFileBuffer, HMODULE pMemoryImage);
	STATUS SetSectionProperty(HANDLE hProcess, void* pFileBuffer, HMODULE pMemoryImage);
